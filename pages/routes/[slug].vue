    ...
    <section class="route-hero">
      ...
      <div class="container route-hero-content">
        <nav class="route-breadcrumbs" :aria-label="text.navAria">
          ...
          <NuxtLink :to="localePath('/#routes')">{{ text.home.routesTitle }}</NuxtLink>
...
const { text, businessText, routeOffers, bookingTerms, reviews, localePath } = useLocaleContent()
...
const routeUrl = new URL(assetPath(route.path), siteUrl).toString()
      ...
      innerHTML: JSON.stringify({
        ...
        itemListElement: [
          ...
          },
          {
            '@type': 'ListItem',
            position: 2,
            name: text.value.home.routesTitle,
            item: new URL(assetPath(localePath('/#routes')), siteUrl).toString()
          },
          {
            '@type': 'ListItem',
            position: 3,
    ...
    ...(initialOffer.routeFaq?.length
      ? [{
          ...
          innerHTML: JSON.stringify({
            '@context': 'https://schema.org',
            '@type': 'FAQPage',
            mainEntity: initialOffer.routeFaq.map((item) => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: {
                '@type': 'Answer',
